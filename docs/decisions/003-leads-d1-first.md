...
**Update 2026-09-24 (#109):** newsletter subscribers follow the same pattern, sharing `outbox_status` (`ref_kind = 'subscriber'`). Migrations stay additive so a rolled-back Worker still runs on the new schema; widening a CHECK rule (a table rebuild, as in `0003_subscribers.sql`) counts as additive because every row the old code writes is still accepted.
